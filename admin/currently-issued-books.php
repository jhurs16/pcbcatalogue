...
if (isset($_SESSION['RollNo'])) {
    ...
    $db->query("SELECT br.*, b.Title AS bookname, DATEDIFF(CURDATE(), br.duedate) AS overdue_days 
...
WHERE STATUS = 'borrowed';");
    ...
    function calcDue($issue, $due)
    {
        if ($issue && $due) {
            // $issueTimestamp = strtotime($issue);
            // $dueTimestamp = strtotime($due);

            // // Calculate the difference in seconds and convert to days
            // $differenceInSeconds = $dueTimestamp - $issueTimestamp;
            // $daysUntilDue = $differenceInSeconds / (60 * 60 * 24); // Convert seconds to days
            // $displayDaysUntilDue = $daysUntilDue < 0 ? abs($daysUntilDue) . " (Overdue)" : $daysUntilDue;
            // // Ensure daysUntilDue is not negative
            // echo $displayDaysUntilDue;
            // // echo $daysUntilDue = $daysUntilDue >= 0 ? $daysUntilDue : 0;

            // Convert issue and due dates to timestamps
            $issueTimestamp = strtotime($issue);
            $dueTimestamp = strtotime($due);
            ...
            // Calculate the difference in days between issue and due date
            $differenceInSeconds = $dueTimestamp - $issueTimestamp;
            $daysUntilDue = $differenceInSeconds / (60 * 60 * 24);

            // Calculate the difference in days between due date and current date
            $currentDifferenceInSeconds = $currentTimestamp - $dueTimestamp;
            $daysSinceDue = $currentDifferenceInSeconds / (60 * 60 * 24);

            // Display results
            //echo "Days from issue to due date: " . $daysUntilDue . "<br>";
            if ($daysSinceDue > 0) {
                // echo "Days since due date: " . round($daysSinceDue) . "<br>";
                echo round($daysSinceDue);
            } else {
                //echo "Days remaining until due date: " . abs(round($daysSinceDue)) . "<br>";
                echo abs(round($daysSinceDue));
            }
        } else {
            echo 0;
        }
...
?>
    ...
    <body class="layout-boxed">
                    ...
                    <ul class="list-unstyled menu-categories" id="accordionExample">
                        ...
                        <li class="menu active">
                            ...
                            <ul class="collapse submenu list-unstyled show" id="pages" data-bs-parent="#accordionExample">
                                ...
                                <li class="active">
                                    <a href="./currently-issued-books"> Currently Borrowed Books </a>
                                </li>
                        ...
                        <div class="page-meta">
                            <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Currently Borrow Books</li>
                                </ol>
                            </nav>
                        </div>
                                        ...
                                        <table id="style-3" class="table style-3 dt-table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($current_records as $current):
                                                    $rollno = $current['userid'];
                                                    $bookid = $current['bookid'];
                                                    $name = $current['bookname'];
                                                    $issuedate = $current['borrowdate'];
                                                    $duedate = $current['duedate'];
                                                    $dues = $current['overdue_days'];
                                                ...
                                                ?>
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <?php convertDate($duedate); ?>
                                                        </td>
                                                        <td class="">
                                                            <span class="shadow-none badge badge-<?php echo $dues > 0 ? 'danger' : 'success'; ?>" id="duespan">
                                                                <?php echo $dues > 0 ? $dues : 0; ?>
                                                                <!-- <?php calcDue($issuedate, $duedate); ?> -->
                                                            </span>
        ...
        <script>
            ...
            function calculateDues(issueDate, dueDate) {
                ...
                // Display the result in the 'days' input
                return document.getElementById("days").value = daysUntilDue >= 0 ? daysUntilDue : '';
            ...
            function issueBook(bookId, rollno, actions) {


                $.ajax({
                    url: actions === 'issue' ? './sql-actions/return-book-current.php' : './sql-actions/renew-book-current.php',
                    type: 'GET',
                    data: {
                        bookId,
                        rollno
